...
(function () {
    ...
    return `
      <article class="card soft" data-id="${escapeAttr(item.id)}">
        <div class="card-header">
          <div>
            <div class="muted small">${escapeHtml(item.range)} • ${escapeHtml(
      item.stage
    )}</div>
  ...
  function openDetails(item) {
    ...
    setTimeout(() => {
      ...
      closeBtn?.addEventListener("click", () => window.Barca?.closeModal());

      const tp = document.getElementById("tp-milestone");
      tp?.addEventListener("click", () => {
        cpRumTag("tracepoint", "milestone", item.id);
        window.Barca?.toast(`Tracepoint sent: milestone=${item.id}`, "ok", {
          ttl: 2200,
        });
      });
    }, 0);
  ...
  async function loadData() {
    ...
    try {
      ...
      window.Barca?.toast(
        "Failed to load legends.json (see console).",
        "danger"
      );
      console.error(e);
    } finally {
