...
(function () {
  ...
  function getCart() {
    return window.CPRUM?.getCart() || { items: [] };
  }

  function saveCart(cart) {
    window.CPRUM?.setCart(cart);
  }
  ...
  function addToCart(id) {
    ...
    window.CPRUM?.toast(`${p.name} added to cart`, 'ok', { ttl: 1800 });
  ...
  function renderProducts() {
      ...
      .map((p) => {
        return `
          <article class="card product-card">
            <img src="${escapeHtml(p.img)}" width="640" height="420" alt="${escapeHtml(p.name)} (custom SVG)" />
            <div class="card-header" style="margin-top:10px;">
              <div>
                <h3 class="card-title">${escapeHtml(p.name)}</h3>
                <div class="rating">${escapeHtml(p.tag)} • ${escapeHtml(p.rating)}★</div>
              </div>
              <div class="price">${money(p.price)}</div>
            </div>
            <p class="muted">${escapeHtml(p.desc)}</p>
            <div class="btn-row">
              <button class="btn btn-primary" type="button" data-add="${escapeHtml(p.id)}" data-tooltip="Stores cart in localStorage">
                Add to cart
              </button>
              <button class="btn btn-ghost" type="button" data-details="${escapeHtml(p.id)}">
                Details
              </button>
            </div>
          </article>
    ...
    els.grid.querySelectorAll('[data-details]').forEach((b) => {
      b.addEventListener('click', () => showDetails(b.getAttribute('data-details')));
    });

    window.CPRUM?.initTooltips();
  }
  ...
  function showDetails(id) {
    ...
    const body = `
      <div class="grid grid-2">
        <div>
          <img src="${escapeHtml(p.img)}" alt="${escapeHtml(p.name)}" style="width:100%; border-radius:16px; border:1px solid var(--border);" />
        </div>
        <div>
    ...
    window.CPRUM?.openModal({ title: 'Product detail', body });
    setTimeout(() => {
      document.getElementById('modal-add')?.addEventListener('click', () => addToCart(id));
      document.querySelector('.modal [data-close]')?.addEventListener('click', () => window.CPRUM?.closeModal());
    }, 0);
  }
          ...
          return `
            <div class="cart-item" data-id="${escapeHtml(it.id)}">
              ...
              <div class="meta">
                ...
                <div class="price">${money(it.price)} • <span class="muted">Qty</span>
                  <input type="number" min="1" value="${escapeHtml(it.qty)}" data-qty style="width:78px; margin-left:6px;" />
                </div>
              </div>
  ...
  function wireCheckout() {
    els.checkout?.addEventListener('submit', (e) => {
      ...
      const items = cart.items || [];
      if (!items.length) {
        window.CPRUM?.toast('Cart is empty. Add items before checkout.', 'danger');
        return;
      }
      ...
      window.CPRUM?.toast('Processing… (fake)', 'info', { ttl: 1200 });

      setTimeout(() => {
        const dur = performance.now() - start;
        cpRumTag('tracepoint', 'checkout', `submitted:${Math.round(dur)}ms`);
        window.CPRUM?.toast('Order placed! (Not really — static site)', 'ok', { ttl: 2600 });
  ...
  async function loadProducts() {
    ...
    try {
      ...
      window.CPRUM?.toast('Failed to load products.json (see console).', 'danger');
      console.error(e);
    } finally {
